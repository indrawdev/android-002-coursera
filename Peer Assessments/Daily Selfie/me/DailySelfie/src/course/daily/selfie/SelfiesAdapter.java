...
public class SelfiesAdapter extends CursorAdapter {
	...
	public SelfiesAdapter(Context context, Cursor cursor, int flags) {
		...
		mBitmapStoragePath = SelfiesPictureHelper
				.getBitmapStoragePath(mContext);
	...
	public void addSelfie(SelfieRecord newSelfie) {
		...
		if (SelfiesPictureHelper.storeBitmapToFile(
				newSelfie.getPictureBitmap(), filePath)) {
			newSelfie.setPicturePath(filePath);
		}

		mSelfiesDatabase.insertSelfie(newSelfie.getName(),
				newSelfie.getPicturePath());
	...
	private SelfieRecord getSelfieRecordFromCursor(Cursor cursor) {
		...
		int id = cursor
				.getInt(cursor.getColumnIndex(SelfiesDatabase.KEY_ROWID));
		String name = cursor.getString(cursor
				.getColumnIndex(SelfiesDatabase.KEY_NAME));
		String picturePath = cursor.getString(cursor
				.getColumnIndex(SelfiesDatabase.KEY_PICTURE_PATH));
	...
	public void bindView(View view, Context context, Cursor cursor) {
		...
		int dimenPix = (int) mContext.getResources().getDimension(
				R.dimen.selfie_row_picture_width_height);

		holder.picture.setImageBitmap(SelfiesPictureHelper.getScaledBitmap(
	...
	public View newView(Context context, Cursor cursor, ViewGroup parent) {
		...
		View newView = mLayoutInflater.inflate(R.layout.selfie_row, parent,
				false);
		holder.name = (TextView) newView
				.findViewById(R.id.selfie_name_text_view);
		holder.picture = (ImageView) newView
				.findViewById(R.id.selfie_picture_image_view);
